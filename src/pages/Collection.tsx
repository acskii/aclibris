...
import { ArrowLeft, Book, Library, FileText, Calendar, User, Hash, Eye } from 'lucide-react';
      ...
      {error && (
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-indigo-900 to-violet-900 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-red-300 text-lg mb-4">{error || 'Collection not found'}</p>
            <button 
              type="button"
              onClick={goBack}
            ...
            {selectedBook ? (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {selectedBook.author && (
                    <p className="text-indigo-200 text-lg mb-3">by {selectedBook.author}</p>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center py-2 border-b border-indigo-500/20">
                    ...
                    <span className="text-white font-semibold">{selectedBook.pages}</span>
                  </div>
                  
                  <div className="flex justify-between items-center py-2 border-b border-indigo-500/20">
                    <span className="text-indigo-200">File Size</span>
                    <span className="text-white font-semibold">{formatFileSize(selectedBook.fileSize)}</span>
                  </div>
                  
                  <div className="flex justify-between items-center py-2 border-b border-indigo-500/20">
                    <span className="text-indigo-200">Created at</span>
                    <span className="text-white font-semibold">{fromUnix(selectedBook.createdAtInUnix)}</span>
                  </div>
                </div>
          ...
          <div className="bg-gray-800/30 backdrop-blur-sm rounded-md p-6 border border-indigo-500/20">
            ...
            {books.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-indigo-200 text-lg">No books in this collection yet</p>
